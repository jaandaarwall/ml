...
def seed_dummy_db():
    with app.app_context():
        ...
        # --------------------------
        department_names = [
            ("Cardiology", "Heart & vessels", 500.0),
            ("Neurology", "Brain & nerves", 400.0),
            ("Orthopedics", "Bones & joints", 350.0),
            ("Dermatology", "Skin & hair", 300.0),
        ]

        departments = []
        for name, desc, price in department_names:
            dept = Department(name=name, description=desc, price=price)
            db.session.add(dept)
            departments.append(dept)
        ...
        for i, patient in enumerate(patients):
            ...
            # Create a payment for the completed appointment
            payment = Payment(
                appointment_id=apt2.id,
                amount=departments[doctor.department_id - 1].price,  # Assuming department_id starts from 1
                status="Success"
            )
